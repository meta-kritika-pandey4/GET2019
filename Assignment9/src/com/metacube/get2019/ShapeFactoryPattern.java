...
public class ShapeFactoryPattern {
		...
		try {
			do {
				...
				case 1:
					...
					y = sc.nextInt();
					p = new Point(x, y);
					parameter = getParameter( shapeName, parameter);
					Shape shape1 = shapeFactory.createShape(shapeName, p, parameter);
	...
	private static ArrayList<Integer> getParameter(String shapeName, ArrayList<Integer> parameter) {
		shapeName = shapeName.toUpperCase();
		switch (shapeName) {
		case "CIRCLE":
			System.out.print("Enter radius of circle : ");
			int radius = sc.nextInt();
			parameter.add(radius);
			break;
		case "RECTANGLE":
